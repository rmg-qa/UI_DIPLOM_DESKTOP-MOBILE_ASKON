...
def test_checking_the_global_search_output(setup_browser):
    main_page.open_the_page()
    main_page.search_product_from_title_bar(input_text='Коврик для ванной')
    main_page.global_search_product_result(result='Коврик для ванной')
...
def test_add_item_to_cart_unauthorized_user(setup_browser):
    main_page.open_the_page()
    main_page.search_product_from_title_bar(input_text='диван')
...
def test_choosing_city_in_the_header(setup_browser):
    main_page.open_the_page()
    main_page.search_and_select_city_in_header(input_text='Кемерово')
    main_page.checking_display_selected_city(result='Кемерово')
...
def test_filtering_products_by_price(setup_browser):
    ...
    main_page.checking_display_filtered_product_by_price(price_from=5000, price_up_to=10000)
